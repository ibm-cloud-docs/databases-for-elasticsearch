...
- Access to the Account, Management Console UI, and API is secured via [Identity and Access Management (IAM)](/docs/databases-for-elasticsearch?topic=cloud-databases-iam).
...
- IP Whitelisting - All deployments support [whitelisting IP addresses](/docs/databases-for-elasticsearch?topic=cloud-databases-whitelisting) to restrict access to the service.
...
- {{site.data.keyword.databases-for-elasticsearch}} deployments are configured with replication. Deployments contain a cluster with three nodes where all three are data nodes and any node can be the master node. You can also [add nodes](/docs/databases-for-elasticsearch?topic=databases-for-elasticsearch-horizontal-scaling) to provide additional stability in the event of a multi-node failure, since you can lose more nodes and maintain a quorum.
